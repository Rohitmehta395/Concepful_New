...
export function AiCollaborationClient() {
  const { data: profile, isLoading } = useGetAiProfile(1, { query: { queryKey: ["ai-profile", 1] } as any });
  ...
  const handleSave = () => {
    upsertProfile.mutate(
      { companyId: 1, data: { companyId: 1, providers, modelName, useCases, usageNotes, consentAiWorkflows, consentBrandMemory } as any },
      {
        onSuccess: () => {
