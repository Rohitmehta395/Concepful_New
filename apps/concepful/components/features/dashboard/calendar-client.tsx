...
import { loadPings, Ping, KIND_META, SUBTYPE_META, savePings } from "@/lib/dashboard/pings";
...
function getProjectName(projectId?: string): string | null {
  if (typeof window === "undefined") return `Project #${projectId}`;
...
export function CalendarClient() {
  ...
  /* ping update helpers */
  const updatePing = (id: string, patch: Partial<Ping>) => {
    setPings((prev: Ping[]) => {
                ...
                {selectedDayPings.map(ping => (
                  <HistoryRow
                    ...
                    onClick={() => ping.projectId
                      ? router.push(`/dashboard/project/${ping.projectId}?pingId=${ping.id}`)
                      : setSelected(ping)
                    }
                  />
                ))}
