...
async function getCredentials(): Promise<{ publishableKey: string; secretKey: string }> {
  ...
  if (!hostname || !xReplitToken) {
    throw new Error('Stripe integration not configured. Connect via the Integrations tab.');
  }
  ...
  if (!settings?.secret || !settings?.publishable) {
    throw new Error('Stripe credentials missing — ensure the integration is connected.');
  }
